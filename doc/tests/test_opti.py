...
for b in m.bars:
  if b.tension != 0.:
    b.section = b.tension / yield_stress

def func(X):
  for i in xrange(len(m.bars)): m.bars[i].section = abs(X[i])
  m.solve()
  e = (np.array([abs(b.stress) - yield_stress for b in m.bars])**2).sum()  
  return e
 
X0 = np.array([b.section for b in m.bars])
